...
// When the waiter asls the coffee machine to make a coffee again,
...
class CoffeeMachine extends Thread {
		...
		public void run() {
				while(true) {
						makeCoffee();
						try {
								System.out.println("Coffee machine: Making anohter coffee now");
								// simulate the time take to make a coffe by calling sleep method
								Thread.sleep(1000);
						} catch(InterruptedException e) {
								// Its okay to ignore this exception
								// since we're not using thread interrup mechanism
								e.printStackTrace();
						}
				}
		}
